      ...
      <div className="min-h-[50vh] flex flex-col items-center justify-center text-center p-6 space-y-4 max-w-md mx-auto animate-fade-in">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-sm leading-relaxed">
            {error?.response?.data?.message || error?.message || "We encountered an issue retrieving your medical record history. Please try again."}
          </p>
        </div>
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto p-4 sm:p-6">
      <div className="flex flex-col gap-1">
        <h2 className="text-xl sm:text-2xl font-extrabold text-foreground">Medical Records History</h2>
        <p className="text-xs text-muted-foreground">
          View all diagnostic, treatment, and advice files compiled by your consultants.
        </p>
      </div>
      ...
      {medicalRecords.length === 0 ? (
        ...
        <Card className="border border-dashed border-border/40 shadow-2xs bg-card p-12 text-center rounded-2xl animate-fade-in">
          ...
          <h3 className="text-base font-bold text-foreground mb-1.5">No Medical Records</h3>
          <p className="text-xs text-muted-foreground max-w-xs mx-auto leading-relaxed">
            You do not have any clinical logs or medical records registered in the system yet.
          </p>
        </Card>
      ) : (
        // Grid Listing
              ...
              >
                ...
                <CardContent className="p-5 space-y-4 flex-1">
                  ...
                  <div className="flex gap-2.5 items-start text-xs sm:text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-extrabold text-foreground mt-0.5">
                        {record.doctor?.name?.startsWith("Dr.") ? record.doctor.name : `Dr. ${record.doctor?.name}`}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-2.5 border-t border-border/5">
                    <div className="space-y-0.5 text-xs sm:text-sm">
                      <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Diagnosis</span>
                      <p className="text-muted-foreground line-clamp-1 mt-0.5">
                        {record.diagnosis || "N/A"}
                      </p>
                    </div>
                    <div className="space-y-0.5 text-xs sm:text-sm">
                      <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Treatment</span>
                      <p className="text-muted-foreground line-clamp-1 mt-0.5">
