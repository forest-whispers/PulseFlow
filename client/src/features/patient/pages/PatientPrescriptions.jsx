...
import { Calendar, User, Pill, Stethoscope, RefreshCw, AlertCircle, Eye, Info } from "lucide-react"
      ...
      <div className="min-h-[50vh] flex flex-col items-center justify-center text-center p-6 space-y-4 max-w-md mx-auto animate-fade-in">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-sm leading-relaxed">
            {error?.response?.data?.message || error?.message || "We encountered an issue retrieving your prescription history. Please try again."}
          </p>
        </div>
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto p-4 sm:p-6">
      <div className="flex flex-col gap-1">
        <h2 className="text-xl sm:text-2xl font-extrabold text-foreground">Prescription History</h2>
        <p className="text-xs text-muted-foreground">
          View all prescriptions issued during your medical consultations.
        </p>
      </div>
      ...
      {prescriptions.length === 0 ? (
        ...
        <Card className="border border-dashed border-border/40 shadow-2xs bg-card p-12 text-center rounded-2xl animate-fade-in">
          ...
          <p className="text-xs text-muted-foreground max-w-xs mx-auto leading-relaxed">
            You do not have any prescriptions registered in the system yet.
          </p>
        </Card>
      ) : (
        // Grid Listing
                  ...
                  <CardContent className="p-5 space-y-4 flex-1">
                    ...
                    {prescription.medicalRecord?.chiefComplaint && (
                      <div className="space-y-0.5 text-xs sm:text-sm">
                        ...
                        <p className="text-foreground font-semibold line-clamp-1 mt-0.5 leading-relaxed">
                          {prescription.medicalRecord.chiefComplaint}
                        </p>
                      </div>
                    )}
